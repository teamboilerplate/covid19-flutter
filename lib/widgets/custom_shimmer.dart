...
class Shimmer extends StatefulWidget {
  static final Gradient defaultGradient = LinearGradient(colors: [
    AppColors.offBlackColor.withOpacity(0.9),
    AppColors.offWhiteColor.withOpacity(0.3),
  ]);
  final Widget child;
  final bool rrect;
  final Size rrectSize;
  ...
  const Shimmer({
    ...
    this.enabled = true,
    this.radius = 10,
    this.rrectSize = const Size(100, 10),
  }) : super(key: key);
...
class _ShimmerState extends State<Shimmer> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.rrect) {
      child = ClipRRect(
        ...
        child: Container(
          height: widget.rrectSize.height,
          width: widget.rrectSize.width,
          color: AppColors.offBlackColor.withOpacity(0.9),
        ),
      );
    } else {
      child = widget.child;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (BuildContext context, Widget xchild) => _Shimmer(
